...
class SimpleLoginSecure
{
	 ...
	 * @return	bool
	 */
	function update($user_email = '', $user_pass = '') 
	{
		$this->CI =& get_instance();
		...
		if ($query->num_rows() < 1) //user_email not found
			return false;
	...
	function login($user_email = '', $user_pass = '') 
	{
		...
		if ($query->num_rows() > 0) 
		{
			...
			$this->CI->db->simple_query('UPDATE ' . $this->user_table  . ' SET user_last_login = NOW() WHERE user_id = ' . $user_data['UserID']);
